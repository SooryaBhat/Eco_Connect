...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SESSION_SECRET', 'your-secret-key-change-this')
...
def login():
    ...
    if request.method == 'POST':
        ...
        user = db_session.query(User).filter_by(username=username, role=role).first()
...
def scan_bin():
    ...
    if bin_data:
        return jsonify({
            'success': True,
            'bin': {
                'code': bin_data.bin_code,
                'location': bin_data.location,
                'recyclable_weight': bin_data.recyclable_weight,
                'non_recyclable_weight': bin_data.non_recyclable_weight,
                'fill_percentage': round(bin_data.get_fill_percentage(), 2),
                'last_cleaned': bin_data.last_cleaned.strftime('%Y-%m-%d') if bin_data.last_cleaned else 'Never',
...
def user_sell():
    ...
    recyclers = db_session.query(User).filter_by(role='Recycler', location=current_user.location).all()
...
def collector_dashboard():
    bins = db_session.query(Bin).filter_by(location=current_user.location).all()
...
def collector_complaints():
    complaints = db_session.query(Complaint).filter_by(location=current_user.location).all()
...
def recycler_complaint():
    ...
    my_complaints = db_session.query(Complaint).filter_by(user_id=current_user.id).all()
...
def government_transactions():
    transactions = db_session.query(WasteSale).filter_by(location=current_user.location).all()
    return render_template('government/transactions.html', transactions=transactions)
